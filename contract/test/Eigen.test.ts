...
describe('Eigen tests', () => {
  ...
  describe('#constructor', () => {
    it('should set the initial supply correctly', async () => {
      ...
      expect(await eigen.balanceOf(wallet0.address)).to.eq(
        BigNumber.from(1000),
      );
    });
    ...
    it('should revert if initial supply is zero', async () => {
      ...
      await expect(
        eigenFactory.deploy(BigNumber.from(0), overrides),
      ).to.be.revertedWith('Initial supply must be greater than zero');
    });
  });

  describe('#claimReward', () => {
    it('should allow the owner to claim rewards for a user', async () => {
      await expect(
        eigen.connect(wallet0).claimReward(rewardAmount, wallet1.address),
      )
    ...
    it('should revert if rewards have already been claimed', async () => {
      ...
      await expect(
        eigen.connect(wallet0).claimReward(rewardAmount, wallet0.address),
    ...
    it('should revert if rewards amount is zero', async () => {
      ...
      await expect(
        eigen.connect(wallet0).claimReward(zeroRewardAmount, wallet0.address),
      ).to.be.revertedWith('No rewards to claim');
    });

    it('should revert if a non-owner tries to claim rewards', async () => {
      await expect(
        eigen.connect(wallet1).claimReward(rewardAmount, wallet1.address),
